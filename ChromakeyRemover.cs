...
namespace dkxce
{
    ...
    // Multiple YCbCr Colors with same treshold
    public class YCbCrMultiChromakeyRemover
    {
        private Color[] base_colors = new Color[] { Color.Green };
        private Color? transparent_color = null;
        private bool full_mask = false;
        private int[][] mask_colors = new int[][] { new int[] { 150, 44, 21 } };
        private byte min_treshold = 8;
        private byte max_treshold = 96;
        ...
        public Color[] MaskColors
        {
            ...
            {
                base_colors = value;
                if (base_colors == null || base_colors.Length == 0) base_colors = new Color[] { Color.Green };
                mask_colors = new int[base_colors.Length][];
                for(int i = 0; i< base_colors.Length; i++)
                    mask_colors[i] = new int[] { rgb2y(base_colors[i].R, base_colors[i].G, base_colors[i].B), rgb2cb(base_colors[i].R, base_colors[i].G, base_colors[i].B), rgb2cr(base_colors[i].R, base_colors[i].G, base_colors[i].B) };
            }
            get
            {
                return (Color[])base_colors.Clone();
            }
        }
        public Color? TransparentColor { set { transparent_color = value; } get { return transparent_color; } }
        public bool FullMask { set { full_mask = value; } get { return full_mask; } }
        ...
        private double colorclose(int R, int G, int B, out int OR, out int OG, out int OB)
        {
            OR = R; OG = G; OB = B;
            double max_dist = double.MaxValue;
            for(int i=0;i<mask_colors.Length;i++)
            {
                double d = Math.Sqrt(Math.Pow(mask_colors[i][1] - rgb2cb(R, G, B), 2) + Math.Pow(mask_colors[i][2] - rgb2cr(R, G, B), 2));
                if (d < max_dist)
                {
                    max_dist = d;
                    OR = R;
            ...
            return 1 - (max_dist < min_treshold ? 0 : max_dist > max_treshold ? 1 : (max_dist - min_treshold) / (max_treshold - min_treshold));
        }
        ...
        public (int, int, byte[]) RemoveChromaKey2Bytes(Bitmap image)
        {
            ...
            if (transparent_color.HasValue)
            {
                var pixels = Enumerable.Range(0, rgba_foreground.Length / 4).Select(x => new
                {
                    ...
                    G = rgba_foreground[(x * 4) + 1],
                    R = rgba_foreground[(x * 4) + 2],
                    MakeTransparent = new Action<double, int, int, int>((mask, R, G, B) =>
                    {
                        if (full_mask)
                        {
                            rgba_foreground[(x * 4)] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4)] - mask * B, 0) + mask * transparent_color.Value.B), 255); //B
                            rgba_foreground[(x * 4) + 1] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 1] - mask * G, 0) + mask * transparent_color.Value.G), 255); //G
                            rgba_foreground[(x * 4) + 2] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 2] - mask * R, 0) + mask * transparent_color.Value.R), 255); //R
                        };
                        rgba_foreground[(x * 4) + 3] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 3] - mask * 255, 0) + mask * transparent_color.Value.A), 255);     //A
                    })
                });
                pixels.AsParallel().ForAll(pixel => pixel.MakeTransparent(colorclose(pixel.R, pixel.G, pixel.B, out int OR, out int OG, out int OB), OR, OG, OB));
            }
            else
            {
                var pixels = Enumerable.Range(0, rgba_foreground.Length / 4).Select(x => new
                {
                    ...
                    MakeTransparent = new Action<double,int,int,int>((mask, R, G, B) =>
                    {
                        if (full_mask)
                        {
                            rgba_foreground[(x * 4)] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4)] - mask * B, 0) + mask * 0), 255); //B
                            rgba_foreground[(x * 4) + 1] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 1] - mask * G, 0) + mask * 0), 255); //G
                            rgba_foreground[(x * 4) + 2] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 2] - mask * R, 0) + mask * 0), 255); //R
                        };
                        rgba_foreground[(x * 4) + 3] = (byte)Math.Min((Math.Max(rgba_foreground[(x * 4) + 3] - mask * 255, 0) + mask * 0), 255);               //A
                    })
                });
        ...
        {
            string current_path = Directory.GetCurrentDirectory();
            YCbCrMultiChromakeyRemover converter = new YCbCrMultiChromakeyRemover()
            {
                FullMask = true,
                MaskColors = new Color[] { Color.FromArgb(21, 238, 211), Color.FromArgb(255, 38, 40) },
                TransparentColor = Color.Fuchsia,
                MinTreshold = 50,
                MaxTreshold = 82
            };
            string suffix = $"_(ycbcrm_{converter.MinTreshold}_{converter.MaxTreshold})";
            Bitmap source_image = (Bitmap)Bitmap.FromFile(Path.Combine(current_path, "greenbox.jpg"));
